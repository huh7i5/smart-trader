"""
core/pre_trade_checklist.py â€?Automated 3-Point Pre-Trade Checklist

Runs the three mandatory checks before any trade:
  â‘?Smart Money Direction (ETF flows + whale signals)
  â‘?Retail Behavior (Taker volume + order book depth)
  â‘?Macro Events (upcoming CPI, FOMC, earnings)

Only proceed when all three checks pass (ðŸŸ¢ðŸŸ¢ðŸŸ¢).
...
def check_smart_money(ex, sym="BTC/USDT"):
    """
    Check â‘?â€?Smart Money Direction.
    ...
    status = "ðŸŸ¢ PASS" if is_bullish else "ðŸ”´ CAUTION"

    print(f"\n  â‘?Smart Money Direction: {status}")
    ...
    if is_bullish:
        print("     â†?Smart money appears to be accumulating")
    else:
        print("     â†?Smart money direction unclear or bearish")
...
def check_retail_behavior(ex, sym="BTC/USDT", hours=6):
    ...
    status = "ðŸŸ¢ PASS" if is_favorable else "ðŸ”´ CAUTION"

    print(f"\n  â‘?Retail Behavior ({hours}h): {status}")
    print(f"     Net flow: {'ðŸŸ¢ NET BUY' if is_net_buy else 'ðŸ”´ NET SELL'}")
    print(f"     Price change: {price_change:+.2f}%")
    if retail_selling_price_stable:
        print("     â†?Retail is selling but price is stable = bottom signal (smart money absorbing)")
    elif is_net_buy:
        print("     â†?Retail is buying, positive sentiment")
    else:
        print("     â†?Retail is selling AND price is dropping = risk")
...
def check_macro_events():
    """
    Check â‘?â€?Macro Events Calendar.
    ...
    print(f"\n  â‘?Macro Events (next 48h): {status}")
    if upcoming:
        for hours_until, desc, risk in sorted(upcoming):
            risk_emoji = {"CRITICAL": "ðŸ”´", "HIGH": "ðŸŸ¡", "MEDIUM": "ðŸŸ¢"}.get(risk, "âš?)
            print(f"     {risk_emoji} {desc} â€?in {hours_until:.1f} hours ({risk})")
    else:
...
def run_checklist(symbol="BTC/USDT"):
    ...
    print(f"\n{'=' * 60}")
    print(f"  VERDICT: {'âœ?ALL CLEAR â€?SAFE TO TRADE' if all_pass else 'â?DO NOT TRADE â€?CONDITIONS NOT MET'}")
    print(f"{'=' * 60}")

    checks = [
        ("â‘?Smart Money", check1),
        ("â‘?Retail Behavior", check2),
        ("â‘?Macro Events", check3),
    ]
    for name, passed in checks:
        print(f"  {name}: {'ðŸŸ¢' if passed else 'ðŸ”´'}")

    if not all_pass:
        print("\n  âš ï¸ Recommendation: Wait for all conditions to turn green.")
